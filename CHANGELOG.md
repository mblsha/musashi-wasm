...
- Correct `System.step()` to keep the post-instruction PC reported by Musashi so control-flow opcodes (e.g., `jsr`) advance into their callees without losing stacked return writes.

### ✅ Tests

- Add a movem + `jsr` regression that asserts the stack delta and callee entry produced by the stepper.
