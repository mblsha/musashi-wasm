...
import type { SystemMemoryIO } from './index';

// Mock System for testing (implements the minimal IO needed)
class MockSystem implements SystemMemoryIO {
...
describe('@m68k/memory', () => {
  ...
  describe('MemoryRegion', () => {
    ...
    test('should throw on size mismatch', () => {
      ...
      expect(() => region.setBytes(wrongSizeData)).toThrow(
        'Data size (10) does not match region size (8)'
      );
    });
  });
  ...
  describe('MemoryArray', () => {
    ...
    test('should iterate over array elements', () => {
      ...
      const values: number[] = [];
      for (const value of array.iterate(1, 3)) {
        // Start at 1, count 3
        values.push(value);
      }
  ...
  describe('DataParser', () => {
    ...
    test('should write unsigned integers', () => {
      ...
      expect(data).toEqual(
        new Uint8Array([0x12, 0x34, 0xab, 0xcd, 0xef, 0x01])
      );
    });
    ...
    test('should write C strings', () => {
      ...
      expect(data.slice(0, 5)).toEqual(
        new Uint8Array([0x54, 0x65, 0x73, 0x74, 0x00])
      );

      DataParser.writeCString(data, 'LongString', 5, 4);
      expect(data.slice(5, 9)).toEqual(
